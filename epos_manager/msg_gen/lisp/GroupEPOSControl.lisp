...
(cl:defclass <GroupEPOSControl> (roslisp-msg-protocol:ros-message)
  ((motor_group
    :reader motor_group
    :initarg :motor_group
    :type (cl:vector epos_manager-msg:EPOSControl)
   :initform (cl:make-array 0 :element-type 'epos_manager-msg:EPOSControl :initial-element (cl:make-instance 'epos_manager-msg:EPOSControl))))
)
...
(cl:defmethod cl:initialize-instance :after ((m <GroupEPOSControl>) cl:&rest args)
  ...
  (cl:unless (cl:typep m 'GroupEPOSControl)
    (roslisp-msg-protocol:msg-deprecation-warning "using old message class name epos_manager-msg:<GroupEPOSControl> is deprecated: use epos_manager-msg:GroupEPOSControl instead.")))

(cl:ensure-generic-function 'motor_group-val :lambda-list '(m))
(cl:defmethod motor_group-val ((m <GroupEPOSControl>))
  (roslisp-msg-protocol:msg-deprecation-warning "Using old-style slot reader epos_manager-msg:motor_group-val is deprecated.  Use epos_manager-msg:motor_group instead.")
  (motor_group m))
...
(cl:defmethod roslisp-msg-protocol:deserialize ((msg <GroupEPOSControl>) istream)
  ...
  (cl:let ((vals (cl:slot-value msg 'motor_group)))
    ...
    (cl:setf (cl:aref vals i) (cl:make-instance 'epos_manager-msg:EPOSControl))
...
(cl:defmethod roslisp-msg-protocol:message-definition ((type (cl:eql '<GroupEPOSControl>)))
  ...
  (cl:format cl:nil "# This represents a control structure for a Maxon motor attached to an EPOS 2 controller.  ~%# The node_id is the node_id set on the epos controller itself~%# The control_mode corresponds with the desired mode of control~%# The setpoint corresponds with the desired value for the selected mode~%~%EPOSControl[] motor_group~%~%================================================================================~%MSG: epos_manager/EPOSControl~%# This represents a control structure for a Maxon motor attached to an EPOS 2 controller.  ~%# The node_id is the node_id set on the epos controller itself~%# The control_mode corresponds with the desired mode of control~%# The setpoint corresponds with the desired value for the selected mode~%~%# Control Mode Options~%uint8 VELOCITY =1~%uint8 ABSOLUTE_POSITION = 2~%uint8 ABSOLUTE_POSITION_IMMEDIATE = 3~%uint8 RELATIVE_POSITION = 4~%uint8 RELATIVE_POSITION_IMMEDIATE = 5~%~%~%uint16 node_id~%uint8 control_mode~%int32 setpoint~%~%~%~%~%~%~%"))
(cl:defmethod roslisp-msg-protocol:message-definition ((type (cl:eql 'GroupEPOSControl)))
  "Returns full string definition for message of type 'GroupEPOSControl"
  (cl:format cl:nil "# This represents a control structure for a Maxon motor attached to an EPOS 2 controller.  ~%# The node_id is the node_id set on the epos controller itself~%# The control_mode corresponds with the desired mode of control~%# The setpoint corresponds with the desired value for the selected mode~%~%EPOSControl[] motor_group~%~%================================================================================~%MSG: epos_manager/EPOSControl~%# This represents a control structure for a Maxon motor attached to an EPOS 2 controller.  ~%# The node_id is the node_id set on the epos controller itself~%# The control_mode corresponds with the desired mode of control~%# The setpoint corresponds with the desired value for the selected mode~%~%# Control Mode Options~%uint8 VELOCITY =1~%uint8 ABSOLUTE_POSITION = 2~%uint8 ABSOLUTE_POSITION_IMMEDIATE = 3~%uint8 RELATIVE_POSITION = 4~%uint8 RELATIVE_POSITION_IMMEDIATE = 5~%~%~%uint16 node_id~%uint8 control_mode~%int32 setpoint~%~%~%~%~%~%~%"))
